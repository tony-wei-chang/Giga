...
var Grid_Panel_Define = 
{
	...
	Initialize : function(View)
	{
		try
		{
			...
			//建立工作列 Header
			Grid_Panel.createToolbar = function()
			{
				var Toolbar = Grid_Panel.getId() + '_Toolbar';
				//如果物件產生過就不重複產生
				if($('#' + Toolbar).length === 0)
				{
					//新增 Toolbar
					$('#'+this.getId()).append("<div id='" + Toolbar + "' class='Mainpage_Main_Toolbar'></div>");
				}		
				//清空 Toolbar
				$('#' + Toolbar).empty();
			}
			...
			Grid_Panel.createHeader = function()
			{
				...
				for(var i=0;i<content.length;i++)
				{
					...
					//產生表格並設定寬度
					if(Grid_Panel.getModel()[i] == "checkbox" || Grid_Panel.getModel()[i] == "CheckBox")
					{
						//核取方塊
						$('#' + Header).append("<div header_width='" + ((Grid_Panel.getHeader_Width()[i] == "0%")?"0%":tdWidth) + "' fieldShow='" + Grid_Panel.getfieldShow()[i] + "' name='" + Grid_Panel.getModel()[i] + "' class='Mainpage_Main_Header_div translateHtml' style='text-overflow:ellipsis;white-space:nowrap;overflow:hidden;width:calc(" + tdWidth + " - 3px);" + fieldShow_Css + "' myId='" + content[i] + "'><input id='" + Grid_Panel.getId() + "_" + Grid_Panel.getModel()[i] + "' type='checkbox' style='width: 20px;height: 20px;cursor: pointer;margin-top: 20px;'></div>");
						//新增全選監聽
						$("#" + Grid_Panel.getId() + "_" + Grid_Panel.getModel()[i]).change(function()
						{
							if($(this).prop("checked"))
							{
								$("#" + Grid_Panel.getId() + " input[name='checkbox']").prop("checked",true);
								//選定其他的選定
								var item = $("#" + Grid_Panel.getId() + "_Table").children();
								for(var i = 0; i < item.length; i++)
								{
									item.eq(i).addClass("Mainpage_Main_Table_Active");
								};
							}
							else
							{
								$("#" + Grid_Panel.getId() + " input[name='checkbox']").prop("checked",false);
								//去除其他的選定
								var item = $("#" + Grid_Panel.getId() + "_Table").children();
								for(var i = 0; i < item.length; i++)
								{
					...
					{
						//一般表格 設定寬度
						$('#' + Header).append("<div header_width='" + ((Grid_Panel.getHeader_Width()[i] == "0%")?"0%":tdWidth) + "' fieldShow='" + Grid_Panel.getfieldShow()[i] + "' id='" + Grid_Panel.getId() + "_" + Grid_Panel.getModel()[i] + "' name='" + Grid_Panel.getModel()[i] + "' class='Mainpage_Main_Header_div translateHtml' style='text-overflow:ellipsis;white-space:nowrap;overflow:hidden;width:calc(" + tdWidth + " - 2px);" + fieldShow_Css + "' myId='" + content[i] + "' title='"+content[i]+"'>" + content[i] + "</div>");
						//是否能夠排序
						if(Grid_Panel.getHeaderSortable())
						{
							//新增排序圖示
							...
							$("#" + Grid_Panel.getId() + "_" + Grid_Panel.getModel()[i] + "_Sort").click(function()
							{
								...
								//由小到大
								else if($(this).attr('sort') == undefined || $(this).attr('sort') == "Desc")
								{
									//改變狀態
									$(this).attr('sort','Asc');
									//刪除舊 Class
				...
				//判定標題是否能移動
				if(Grid_Panel.getHeaderMoveable() && false)
				{
					$("#" + Grid_Panel.getId() + "_Header").sortable
					({
						revert: true,
						axis: "x",
						start:function(e)
						{
							//先隱藏排序圖示
							$("#" + Grid_Panel.getId() + "_Header span").hide();
							//Debug
							$("#" + e.target.id + " .ui-sortable-placeholder").width("calc(" + $("#" + e.toElement.id).attr('header_width') + " - 2px)");
						},
						stop:function(e)
						{
							//移動後 model
							...
							var endHeader = [];
							//新增 Model 資訊
							var endModel_Object = $("#" + Grid_Panel.getId() + "_Header div");
							for(var i = 0; i < endModel_Object.length; i++)
							{
								//新增修改後資訊
								endModel.push(endModel_Object.eq(i).attr('name'));
								endfieldShow.push((endModel_Object.eq(i).attr('fieldShow') == "true")?true:false);
								endHeader_Width.push(endModel_Object.eq(i).attr('header_width'));
								endHeader.push(endModel_Object.eq(i).attr('myid'));
							};
							//更新 GridPanel 資訊
							Grid_Panel.setModel(endModel);
							Grid_Panel.setfieldShow(endfieldShow);
							Grid_Panel.setHeader_Width(endHeader_Width);
							Grid_Panel.setHeader_Title(endHeader);
							Grid_Panel.setData(Grid_Panel.getData());
							//恢復排序圖示
							$("#" + Grid_Panel.getId() + "_Header span").show();
							var Model = Grid_Panel.getModel();
							for(var i = 0; i < Model.length; i++)
							{
								var divId = Grid_Panel.getId() + "_" + Model[i];
								var div_SortId = Grid_Panel.getId() + "_" + Model[i] + "_Sort";
								$("#" + div_SortId).insertAfter($("#" + divId));
							};
						}
				  	});
					$("#" + Grid_Panel.getId() + "_Header").disableSelection();
				}
			}
			//建立 table
			Grid_Panel.createTable = function()
			{
				...
				//新增 Grid_Panel 紀錄
				GridPanel_Object[Grid_Panel.getId()] = Grid_Panel;
				//開始 Window 大小改變監聽
				this.startResize();
			}
			//定義 load 所需欄位
			Grid_Panel.getLoad_Formdata = function()
			{
				return (Grid_Panel.Load_Formdata == undefined)?" ":Grid_Panel.Load_Formdata;
			}
			Grid_Panel.setLoad_Formdata = function(Load_Formdata)
			{
				//定義標題內容
				Grid_Panel.Load_Formdata = Load_Formdata;
			}
			//設定 table 值
			Grid_Panel.load = function()
			{
				...
				var data = (Grid_Panel.getLoad_Formdata() == "")?"":Grid_Panel.getLoad_Formdata();
				//Ajax Get 共用
				jqueryAjax_Post(url,data,function(response)
				{
					//隱藏 Loading
					loadingMask.close();
					//回傳資料
					...
					else
					{
						...
						Mainpage_Main_Msg.addMsgContent('查詢失敗');
				...
				},function(error)
				{
					...
					Mainpage_Main_Msg.show();
					Mainpage_Main_Msg.addMsgContent('查詢錯誤');
			...
			//定義是否能多選 如果沒有設定預設單選
			Grid_Panel.getMulti_Selectable = function()
			{
				return (Grid_Panel.Multi_Selectable == undefined)?false:Grid_Panel.Multi_Selectable;
			}
			Grid_Panel.setMulti_Selectable = function(Multi_Selectable)
			{
				//定義標題內容
				Grid_Panel.Multi_Selectable = Multi_Selectable;
			}
			//定義標題是否能移動
			Grid_Panel.getHeaderMoveable = function()
			{
				return (Grid_Panel.HeaderMoveable == undefined)?false:Grid_Panel.HeaderMoveable;
			}
			Grid_Panel.setHeaderMoveable = function(HeaderMoveable)
			{
				//定義標題內容
				Grid_Panel.HeaderMoveable = HeaderMoveable;
			}
			//定義是否能排序
			Grid_Panel.getHeaderSortable = function()
			{
				return (Grid_Panel.HeaderSortable == undefined)?false:Grid_Panel.HeaderSortable;
			}
			Grid_Panel.setHeaderSortable = function(HeaderSortable)
			{
				//定義標題內容
				Grid_Panel.HeaderSortable = HeaderSortable;
			}
			//設定 table 值
			Grid_Panel.setData = function(Content_Array)
			{
				...
				//恢復 checkBox 沒選狀態
				$("#" + Grid_Panel.getId() + "_Header input").prop("checked",false);
				//設定之前先清空
				$('#' + this.getId() + '_Table').children().remove();
				//依照 Model 資料存入
					...
					for(var i=0;i<Content_Array.length;i++)
					{
						...
						for(var j=0;j<This_Model.length;j++)
						{
							...
							if(j == This_Model.length - 1)
							{
								...
								//如果有需要多選
								if(Grid_Panel.getMulti_Selectable())
								{
									//新增滑鼠按下監聽 Ctrl
									$('#' + Current_ID).mousedown(function(e)
									{
										...
										//Shift 多選
										var selectStart_ID = $("#" + Grid_Panel.getId() + "_Table .Mainpage_Main_Table_Active").eq(0).attr('id');
										if(shiftKey && selectStart_ID)
										{
											//取得第一個選擇的項目編號 由上至下搜尋
											...
											{
												//取得第一個選擇的項目編號 由下至上搜尋
												selectStart_ID = $("#" + Grid_Panel.getId() + "_Table .Mainpage_Main_Table_Active").eq($("#" + Grid_Panel.getId() + "_Table .Mainpage_Main_Table_Active").length - 1).attr('id');
											...
											if(selectStart_Number < selectEnd_Number)
											{
												...
												{
													$("#" + Grid_Panel.getId() + "_Table_Inner_" + i).addClass("Mainpage_Main_Table_Active");
													$("#" + Grid_Panel.getId() + "_Table_Inner_" + i).find("input").prop("checked",true);
												};
											}
										...
										var gridPanel_Pagesize = Grid_Panel.getPagesize();
										if(selectItem_Count == gridPanel_Pagesize)
										{
											$("#" + Grid_Panel.getId() + "_Header div[name='CheckBox']").find('input').prop("checked",true);
										}
										else
										{
											$("#" + Grid_Panel.getId() + "_Header div[name='CheckBox']").find('input').prop("checked",false);
										}
									});
								}
								//新增文字不能被選取
								...
								for(var k=0;k<Width.length;k++)
								{
									...
									//設定寬度
									$('#'+Current_ID).append("<div class='translateHtml' Header_Width='" + ((Grid_Panel.getHeader_Width()[k] == "0%")?"0%":tdWidth) + "' fieldShow='" + Grid_Panel.getfieldShow()[k] + "' name='" + Grid_Panel.getModel()[k] + "' style='width:calc(" + tdWidth + " - 1px);line-height:" + this.Table_Lineheight + ";display:inline-block;text-overflow:ellipsis;white-space:nowrap;overflow:hidden;height:60px;" + fieldShow_Css + "' title='"+fieldValue+"'>" + fieldValue + "</div>");
									//紀錄寬度
			...
			Grid_Panel.prevPage = function()
			{
				...
				if(Grid_Panel.getPage() > 1)
				{
					Grid_Panel.setPage(parseInt(Grid_Panel.getPage()) - parseInt(1));
					Grid_Panel.load();
				}
			}
			//下一頁
	...
	Initialize : function()
	{
		try
		{
			...
			Window.createTitle = function()
			{
				...
				// 	Window.close();
				// });
				$('#' + ID).append("<div id='" + ID + "_Title' class='translateHtml' title='" + this.getTitle() + "'>" + this.getTitle() + "</div>");
				$('#' + ID + '_Title').css
				({
					"width":Width,
					"height":"38px",
			...
			Window.addTextfield = function(ID,Label)
			{
				...
				$('#' + ID + "_Group").append("<div class='Font translateHtml' id='" + ID + "_Label' style='text-align:right'>" + Label + "</div>");
			...
			{																 //.valueField .pageSize .searchUrl .searchPlaceholder .searchKey
				...
				new function()
				{
					...
					//定義資料內容
					_Pagging_Object.ComboboxPagging_getPagging_Parameter = function()
					{
						return Window[_Pagging.ID].ComboboxPagging_Parameter;
					}
					_Pagging_Object.ComboboxPagging_setPagging_Parameter = function(Pagging_Parameter)
					{
						//定義標題內容
						Window[_Pagging.ID].ComboboxPagging_Parameter = Pagging_Parameter;
					}
					//定義搜尋起始值
					...
					_Pagging_Object.ComboboxPagging_load = function()
					{
						...
						var isSearch = false;
						var oldSearchKey = '';
						var Pagging_Parameter = _Pagging_Object.ComboboxPagging_getPagging_Parameter();
						//如果有網址才載入
						if(typeof(Pagging_Parameter.Url) == "undefined" || Pagging_Parameter.Url == "")
						{
							return;
						}
						//改變 searchUrl or Url
						if($("#" + ID + '_Combobox_Pagging_Top_Input').size() && $("#" + ID + '_Combobox_Pagging_Top_Input').val().length != 0)
						{
						...
						//無資料時顯示
						$("#" + ID + '_Combobox_Pagging').append("<div class='noSelected' id='" + ID + "_Combobox_Pagging_Info' style='display:block;height: 40px;line-height: 40px;text-align: center;font-size: 17px;'>查無資料</div>");
						//Debug
						$("#" + ID + "_Combobox_Pagging_Info").click(function()
						{
							$("#" + ID).focus();
						});
						//記錄 loading 狀況
						$("#" + ID + '_Combobox_Pagging').append("<div id='" + ID + "_Combobox_Pagging_loadingStatus' loading='true' style='display:none;'></div>");
						//建立 Pagging
						createPagging();
						//建立 Search bar
						createSearchbar();
						...
						//20160923 Giga下拉式選單 例外跑版 下拉式選單 CSS更新
						...
						//20160923 Giga下拉式選單 例外跑版
						$("#" + ID + '_Combobox_Pagging_loadingStatus').attr('loading','true');
						$("#" + ID + "_Combobox_Pagging_Info").html(getDictionary('Window_Data_Loading'));
						//讀取下拉式選單資料
						jqueryAjax_Post(newUrl,null,function(result)
						{
							loadingMask.close();
							//改變 loading 狀態
							$("#" + ID + '_Combobox_Pagging_loadingStatus').attr('loading','false');
							//20160215 應對回傳資訊有可能非陣列 進行更新
							if(JSON.parse(result)[Pagging_Parameter.rootProperty].length)
							{
								data = JSON.parse(result)[Pagging_Parameter.rootProperty];
							}
							else
							{
								data = JSON.parse(result)[Pagging_Parameter.rootProperty];
							...
							if(data.length == 0)
							{
								...
								//顯示查無資料
								$("#" + ID + "_Combobox_Pagging_Info").show();
								$("#" + ID + "_Combobox_Pagging_Info").html("查無資料");
								//無資料執行方法
								if(typeof(Window[_Pagging.ID].ComboboxPagging_NoData) != "undefined")
								{
									Window[_Pagging.ID].ComboboxPagging_NoData();
								}
							}
							else
							{
								//關閉查無資料
								$("#" + ID + "_Combobox_Pagging_Info").hide();
								$("#" + ID + "_Combobox_Pagging_Info").html("資料載入完成");
							...
							//將資料筆數計算成下拉式選單高度
							var comboboxHeight = (data.length)?data.length:comboboxNondata_Row;
							$('#' + ID + '_Combobox_Pagging').css('height',((parseInt(comboboxHeight) + ((comboboxSearchable)?2:1)) * 40 - 1) + "px");
							// //重設高度
							// $('#' + ID + '_Combobox_Pagging').css("height",((parseInt(data.length) + ((comboboxSearchable)?2:1)) * 40 - 1) + "px");
							//20160923 Giga下拉式選單 例外跑版 下拉式選單 CSS更新
							// $("#" + ID + "_Group").css('margin-top','0px');
							$("#" + ID + "_Combobox_Pagging").width(345);
							$("#" + ID + "_Combobox_Pagging_Bottom").children().css('line-height','16px');
				...
				//設定下拉式選單資訊
				_Pagging_Object.ComboboxPagging_setPagging_Parameter(Pagging_Parameter);
				//產生下拉式選單
				new function()
				{
					var This_ID = Window.getId();
					...
					{
						//建立 Window Textfield text
						$('#' + ID + "_Group").append("<input class='Font Window_Textfield' id='" + ID + "'></input>");
					}
					//Group Css
					$('#' + ID + "_Group").css
					({
					...
					//無資料時顯示
					$("#" + ID + '_Combobox_Pagging').append("<div class='noSelected' id='" + ID + "_Combobox_Pagging_Info' style='display:block;height: 40px;line-height: 40px;text-align: center;font-size: 17px;'>" + getDictionary('Window_Pagging_NotLoad') + "</div>");
					//Debug
					$("#" + ID + "_Combobox_Pagging_Info").click(function()
					{
						$("#" + ID).focus();
					...
					({
						'position':'absolute',
						// 'height':((parseInt(Pagging_Parameter.pageSize) + (comboboxSearchable)?2:1) * 40 - 1) + "px",
				...
				new function()
				{
					...
					if(Pagging_Parameter.Url)
					{
						...
						$("#" + ID + "_Combobox_Pagging_Info").html(getDictionary('Window_Data_Loading'));
						//讀取下拉式選單資料
						jqueryAjax_Post(Pagging_Parameter.Url + "&start=" + Window[_Pagging.ID].ComboboxPagging_getStart() + "&limit=" + Window[_Pagging.ID].ComboboxPagging_getPagesize(),null,function(result)
						{
							//改變 loading 狀態
							$("#" + ID + '_Combobox_Pagging_loadingStatus').attr('loading','false');
							//資料轉換成 json
							var data = JSON.parse(result)[Pagging_Parameter.rootProperty];
							Window[_Pagging.ID].ComboboxPagging_setData(data);
							for(var i = 0; i < data.length; i++)
							{
								//下拉式選單顯示內容
							...
							//將資料筆數計算成下拉式選單高度
							var comboboxHeight = (data.length)?data.length:comboboxNondata_Row;
							$('#' + _Pagging.ID + '_Combobox_Pagging').css('height',((parseInt(comboboxHeight) + ((comboboxSearchable)?2:1)) * 40 - 1) + "px");
						},function()
						{
						},function()
						{
				...
				function createPagging()
				{
					...
					//建立 Pagging 內容
					$('#' + ID + '_Combobox_Pagging').append("<div id='" + Pagging + "' class='Mainpage_Main_Pagging Window_Pagging'></div>");
