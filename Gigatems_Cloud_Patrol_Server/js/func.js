...
function ajaxData_withType(view,ajaxType,ajaxData,callback)
{
	if(Table_Name)
	{
		...
		if(ajaxType == "Insert")
		{
			...
			loadingMask.show();
			//Ajax
			jqueryAjax_Post(serverUrl + "Insert_Gigatems.ashx?TableName=" + Table_Name,ajaxData,function(data)
			{
				//loading
				loadingMask.close();
				//result
				...
				{
					if(Table_Name == "Hq_Member")
					{
						//Msg
						normalSucceed_Msg(getDictionary('Giga_Insert_User_Succeed'));
				...
				{
					if(result.Reason.indexOf('找不到或無法存取伺服器') != -1)
					{
						//Msg
						normalError_Msg(getDictionary('Giga_Server_Not_Found'));
					}
					else
					{
						//Msg
		...
		else if(ajaxType == "Delete")
		{
			...
			loadingMask.show();
			//Ajax
			jqueryAjax_Post(serverUrl + "Delete_Gigatems.ashx?TableName=" + Table_Name,ajaxData,function(data)
			{
				//loading
				loadingMask.close();
				//result
		...
		else if(ajaxType == "Update")
		{
			...
			loadingMask.show();
			//Ajax
			jqueryAjax_Post(serverUrl + "Update_Gigatems.ashx?TableName=" + Table_Name,ajaxData,function(data)
			{
				//loading
				loadingMask.close();
				//result
