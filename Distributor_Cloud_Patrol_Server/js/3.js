...
$('#Mainpage_Menu').ready(function()
{
	try
	{
		...
		$("#Mainpage_Menu_Title_3").children().eq(0).click(function()
		{
			...
			//功能按鈕
			$("#3_Function_Row").append('<div class="Window_Cancel" id="3_Insert" style="margin-left: 0px;width: 50px;background-color: rgb(222, 235, 247);color: black;box-shadow: rgb(136, 136, 136) 5px 3px 5px;font-size: 11px;height: 24px;line-height: 24px;cursor: pointer;">'+getDictionary("Insert")+'</div>');
			...
			$("#3_Function_Row").append('<div class="Window_Cancel" id="3_Delete" style="width: 50px;background-color: rgb(222, 235, 247);color: black;box-shadow: rgb(136, 136, 136) 5px 3px 5px;font-size: 11px;height: 24px;line-height: 24px;cursor: pointer;">'+getDictionary("Update")+'</div>');
			...
			//改寫撈資料參數
			var object = "";
			Maintable_3.setLoad_Formdata(object);
			// $("#Maintable_3_Table_Inner_-1_g_name").focus();
			Maintable_3.load();
			//Css 表格捲軸
			$("#Maintable_3_Table").css("height","calc(100% - 152px)");
			$("#Maintable_3").css("height","calc(100% - 40px)");
			//新增按鈕監聽
			$("#3_Insert").click(function()
			{
				//新增畫面
				...
				Window_3.setTitle(getDictionary("Window_Shift_Arrangement_Insert"));
				Window_3.show();
				...
				$("#Maintable_3_Table_Inner_-1_g_name2_valueField").bind("DOMSubtreeModified", function()
				{
					...
					if(comboboxReset_g_name2)
					{
						...
						$("#Maintable_3_Table_Inner_-1_sch_name").val("");
						$("#Maintable_3_Table_Inner_-1_sch_name_valueField").html("");
						$("#Maintable_3_Table_Inner_-1_sch_name_displayField").html("");
					}
				...
				//確認有無區域可選擇
				Window_3['Maintable_3_Table_Inner_-1_region2'].ComboboxPagging_NoData = function()
				{
					if($("#YesNo_Msg").size() == 0)
					{
						normalError_Msg_Withmask($("#Maintable_3_Table_Inner_-1_g_name2").val() + getDictionary("have no site can be selected."));
					}
				};
				//CSS
				...
				//確認有無路線可選擇
				Window_3['Maintable_3_Table_Inner_-1_pat_name2'].ComboboxPagging_NoData = function()
				{
					if($("#YesNo_Msg").size() == 0)
					{
						normalError_Msg_Withmask($("#Maintable_3_Table_Inner_-1_region2").val() + getDictionary("have no patrol route can be selected."));
					}
				};
				//CSS
				...
				{
					//檢查巡邏點有沒有任務
					if($("#Maintable_3_Table_Inner_-1_pat_name2_valueField").html())
					{
						var ajaxData = 
						{
							"g_name":$("#Maintable_3_Table_Inner_-1_g_name2").val(),
							"region":$("#Maintable_3_Table_Inner_-1_region2").val(),
							"pat_name":$("#Maintable_3_Table_Inner_-1_pat_name2").val()
						};
						jqueryAjax_Post(serverUrl + "Distributor_Cloud_Patrol_server/Select_Distributor.ashx?_Y02="+new Date().getTime()+"&start=0&limit=1&TableName=patrol_route_detail&Order_field=Id&Order_Sequence=Asc",JSON.stringify(ajaxData),function(data)
						{
							//result
							...
							if(result.Success == 'true')
							{
								...
								{
									//Msg
									normalError_Msg_Withmask($("#Maintable_3_Table_Inner_-1_region2").val() + getDictionary("have no patrol point can be selected."));
						...
						{
							//Msg
							// normalError_Msg_Withmask('巡檢點資料查詢錯誤');
						},function()
						{
							//Msg
							// normalError_Msg_Withmask('巡檢點資料查詢錯誤');
				...
				{
					if(!$("#Maintable_3_Table_Inner_-1_g_name2").val())
					{
						normalError_Msg_Withmask(getDictionary("Please Insert")+" "+ getDictionary("Shift_Arrangement_Condominium"));
					}
					else if(!$("#Maintable_3_Table_Inner_-1_region2").val())
					{
						normalError_Msg_Withmask(getDictionary("Please Insert")+" "+ getDictionary("Shift_Arrangement_Site"));
					}
					else if(!$("#Maintable_3_Table_Inner_-1_pat_name2").val())
					{
						normalError_Msg_Withmask(getDictionary("Please Insert")+" "+ getDictionary("Shift_Arrangement_Route_Name"));
					}
					else if(!$("#Maintable_3_Table_Inner_-1_sch_name").val())
					{
						normalError_Msg_Withmask(getDictionary("Please Insert")+" "+ getDictionary("Shift_Arrangement_Shift_Name"));
					}
					else if(!$("#Maintable_3_Table_Inner_-1_date_begin").val())
					{
						normalError_Msg_Withmask(getDictionary("Please Insert")+" "+ getDictionary("Shift_Arrangement_Patrol_Date"));
					}
					else if(!$("#Maintable_3_Table_Inner_-1_gd_name").val())
					{
						normalError_Msg_Withmask(getDictionary("Please Insert")+" "+ getDictionary("Shift_Arrangement_Patrol_Person"));
					}
					else
					{
						//{"rou_id":"5","date_begin":"2016-10-29","gd_id":"1","xdate":"2016/10/29 17:53","xname":"張瑋"}
						var ajaxData = {};
						ajaxData.rou_id = $("#Maintable_3_Table_Inner_-1_pat_name2_valueField").html();
						ajaxData.date_begin = $("#Maintable_3_Table_Inner_-1_date_begin").val();
						ajaxData.gd_id = $("#Maintable_3_Table_Inner_-1_gd_name_valueField").html();
						ajaxData.xdate = $("#Maintable_3_Table_Inner_-1_xdate").val();
						ajaxData.xname = $("#Maintable_3_Table_Inner_-1_xname").val();
						ajaxData_withType(Maintable_3,"Insert",JSON.stringify(ajaxData));
						Window_3.close();
					}
				});
				$("#Window_3_Yes").css
				...
				//預設值
				$("#Maintable_3_Table_Inner_-1_xdate").val(getDate(new Date(),'-') + " " + getTime(new Date()));
				$("#Maintable_3_Table_Inner_-1_xname").val(UserName);
				//唯讀設定
				$("#Maintable_3_Table_Inner_-1_xdate").attr('readonly',true);
				$("#Maintable_3_Table_Inner_-1_xname").attr('readonly',true);
				...
				//產生日期選擇
				$("#Maintable_3_Table_Inner_-1_date_begin").datetimepicker({scrollInput:false,minDate:'-1970/01/01',timepicker:false,format:'Y-m-d'});
				//Window Css
			...
			{
				$("#Mainpage_Menu_Title_3").children().click();
			});
			//刪除
			$("#3_Delete").click(function()
			{
				deleteRow(Maintable_3);
			});
			//FK 共用設定
			function fkSetting()
			{
